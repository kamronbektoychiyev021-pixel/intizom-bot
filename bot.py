...
def add_score(uid, val):
    cur.execute("UPDATE users SET score = score + ? WHERE user_id=?", (val, uid))
    conn.commit()
...
while True:
    try:
        ...
        for u in res.get("result", []):
            ...
            if text == "🚀 Start":
                send(chat_id, f"🚀 Welcome {name}\n\n📢 {CHANNEL_LINK}", keyboard())

            elif text == "📝 Challenge":
                set_state(chat_id, "waiting_task")
                send(chat_id, "📝 Send your task:", keyboard())
                ...
                send(chat_id, "🔥 +10 points!", keyboard())
                ...
                send(chat_id, txt, keyboard())

            elif text == "👤 My Score":
                cur.execute("SELECT score FROM users WHERE user_id=?", (chat_id,))
                s = cur.fetchone()
                score = s[0] if s else 0

                send(chat_id, f"👤 {name}\n🏆 Score: {score}", keyboard())
